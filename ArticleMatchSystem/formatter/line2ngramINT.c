...
#define DEF_FORM "%08llX"
#define PROG_NAME "line2ngramINT"
...
void InitOption(OPT *option){
	...
	option->LF = '\n';
	option->FS = '\t';
	option->form = c_calloc_vec(NAME_LEN);
	sprintf(option->form,"%s",DEF_FORM);
	option->check = 0;
}
			...
			if((c == option.LF)){
				if(option.tail == 1){
					for(j=0;j<option.n-1;j++){
						ngram.i = ngram.i >> 8;
						printf("\t"); printf(option.form,ngram.i);
					}
				}
...
int main(int argc, char **argv){
	...
	if((option.n < 1)||(option.n > 8)){
		fprintf(stderr,"available n : 0 < n < 9.\n");
		im_ext++;
	}
	if(im_ext > 0){
		exit(0);
