...
int ExecCommSearchList(const DATA_BUF comm){
	...
	if(hit > 0){
        	sprintf(RsltBuf.buf+RsltBuf.len,"\nHit:\n");
		RsltBuf.len = strlen(RsltBuf.buf);
		if(ExpandBuf(&RsltBuf,EXPAND_BUF_SIZE*num_query_entry) == -1){
			fprintf(stderr,"failed: ExpandBuf().\n");
		}
		for(i=0;i<num_query_entry;i++){
			sprintf(RsltBuf.buf+RsltBuf.len,"%llx,{",hit_poss[i]);
			RsltBuf.len = strlen(RsltBuf.buf);
				sprintf(RsltBuf.buf+RsltBuf.len,"%u,",db_term_entry[i].pos_ID[0]);
				RsltBuf.len = strlen(RsltBuf.buf);
				sprintf(RsltBuf.buf+RsltBuf.len,"%u",db_term_entry[i].pos_POS[0]);
				RsltBuf.len = strlen(RsltBuf.buf);
			for(j=1;j<db_term_entry[i].num_pos;j++){
				sprintf(RsltBuf.buf+RsltBuf.len," %u,",db_term_entry[i].pos_ID[j]);
				RsltBuf.len = strlen(RsltBuf.buf);
				sprintf(RsltBuf.buf+RsltBuf.len,"%u",db_term_entry[i].pos_POS[j]);
				RsltBuf.len = strlen(RsltBuf.buf);
			}
			sprintf(RsltBuf.buf+RsltBuf.len,"}\n");
			RsltBuf.len = strlen(RsltBuf.buf);
