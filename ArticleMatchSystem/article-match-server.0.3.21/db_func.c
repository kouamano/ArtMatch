...
int ArtDBTermBinSearch(unsigned long long int query, DB_TERM_ENTRY *db, unsigned long long int max, unsigned long long int min, unsigned long long int *hit_pos){
	DB(fprintf(stderr,"IN ArtDBTermBinSearch()\n"));
        unsigned long long int target = min;
        unsigned long long int cur_max = max;
        unsigned long long int cur_min = min;
        while(cur_min <= cur_max){
		if((cur_max > max)||(cur_min > max)||(cur_min < min)||(cur_max < min)){
			DB(fprintf(stderr,"HO HIT\n"));
			return(0);
		}
                target = (unsigned long long int)(cur_min + cur_max)/2;
		...
		DB(fprintf(stderr,"min:cur_min::max:cur_max (current pos) (termINT) - %lld:%lld::%lld:%lld (%llu) (%llX)\n",min,cur_min,max,cur_max,target,db[target].termINT));
        }
	DB(fprintf(stderr,"HO HIT\n"));
	DB(fprintf(stderr,"OUT ArtDBTermBinSearch()\n"));
        return(0);
...
int ArtDBTermSkipSearch(unsigned long long int query, DB_TERM_ENTRY *db, unsigned long long int max, unsigned long long int min, unsigned long long int *hit_pos){
	DB(fprintf(stderr,"IN ArtDBTermSkipSearch()\n"));
        unsigned long long int cur_max = max;
        unsigned long long int cur_min = min;
	unsigned long long int target = (unsigned long long int)(cur_min + cur_max)/2;
	//double v_cur_min = (double)(*(db+cur_min)).termINT;
	//double v_cur_max = (double)(*(db+cur_max)).termINT;
	double v_delta = ((*(db+cur_max)).termINT - (*(db+cur_min)).termINT)/(cur_max - cur_min);
	unsigned long long int v_target = (*(db+target)).termINT;
	DB(fprintf(stderr,"min:cur_min::max:cur_max (current pos) (cur termINT) (cur_max termINT) - %lld:%lld::%lld:%lld (%llu) (%llX) (%llX)\n",min,cur_min,max,cur_max,target,db[target].termINT,db[cur_max].termINT));
	DB(fprintf(stderr,"target :%lld:\n",target));
	DB(fprintf(stderr,"before while\n"));
        while(cur_min <= cur_max){
		DB(fprintf(stderr,"in while -- step 1\n"));
		if((cur_max > max)||(cur_min > max)||(cur_min < min)||(cur_max < min)){
			DB(fprintf(stderr,"HO HIT\n"));
			return(0);
		}
		DB(fprintf(stderr,"in while -- step 2\n"));
		v_delta = ((*(db+cur_max)).termINT - (*(db+cur_min)).termINT) / (cur_max - cur_min);
		DB(fprintf(stderr,"db[cur_max].termINT,v_delta :%lld,%f:\n",db[cur_max].termINT,v_delta));
		v_target = (*(db+target)).termINT;
		target = target + (unsigned long long int)( (query - v_target) / v_delta );
		DB(fprintf(stderr,"target :%lld:\n",target));
		DB(fprintf(stderr,"in while -- step 3\n"));
                //target = (unsigned long long int)(cur_min + cur_max)/2;
                if(query == (*(db+target)).termINT){
			*hit_pos = target;
