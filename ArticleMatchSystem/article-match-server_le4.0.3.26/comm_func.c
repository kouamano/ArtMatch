...
int ExecCommList(const DATA_BUF comm){
	...
	for(i=0;i<db_profile.num_term;i++){
		sprintf(TmpBuf.buf,"%x;",db_term_entry[i].termINT);
		TmpBuf.len = strlen(TmpBuf.buf);
		if(TmpBuf.len < RsltBuf.size-RsltBuf.len-EXPAND_BUF_SIZE){
			sprintf(RsltBuf.buf+RsltBuf.len,"%x;",db_term_entry[i].termINT);
			RsltBuf.len = strlen(RsltBuf.buf);
		}else{
...
int ExecCommSearch(const DATA_BUF comm){
	...
	int hit = -1;
	int i;
	sscanf(comm.buf,"Search?%x",&query);
	DB(fprintf(stderr,"search for :%d(D):\n",query));
	...
	if(hit > 0){
		...
		RsltBuf.len = strlen(RsltBuf.buf);
        	sprintf(RsltBuf.buf+RsltBuf.len,"%X,{",db_term_entry[hit_pos].termINT);
		RsltBuf.len = strlen(RsltBuf.buf);
...
int ExecCommSearchList(const DATA_BUF comm){
	...
	//allocation
	queries = ui_calloc_vec(num_queries);
	hit_poss = ui_calloc_vec(num_queries);
	//scan ngrams
	scan = 0;
	query_str_len = 0;
	num_query_entry = 0;
	...
	for(i=0;i<comm.len;i++){
		...
		if(scan > 0){
			if(comm.buf[i] == NGRAM_LIST_DELIM){
				...
				sscanf(query_str,"%x",queries+num_query_entry);
				num_query_entry++;
				query_str[0] = '\0';
				query_str_len = 0;
			}else{
...
int ExecCommShlinkSearchList(const DATA_BUF comm){
	...
	if(hit > 0){
		...
		for(i=0;i<num_query_entry;i++){
			sprintf(RsltBuf.buf+RsltBuf.len,"%X,{",db_term_entry[hit_poss[i]].termINT);
			RsltBuf.len = strlen(RsltBuf.buf);
				sprintf(RsltBuf.buf+RsltBuf.len,"%u,",db_term_entry[hit_poss[i]].pos_ID[0]);
				RsltBuf.len = strlen(RsltBuf.buf);
				sprintf(RsltBuf.buf+RsltBuf.len,"%u",db_term_entry[hit_poss[i]].pos_POS[0]);
